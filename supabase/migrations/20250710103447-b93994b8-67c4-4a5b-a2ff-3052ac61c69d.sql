...
-- Create branded_challenge_participants view for leaderboard
CREATE OR REPLACE VIEW public.branded_challenge_leaderboard AS
SELECT 
  ucp.challenge_id,
  ucp.user_id,
  p.full_name,
  p.profile_picture_url,
  ucp.current_distance,
  ucp.total_activities,
  ucp.completed_at,
  c.target_distance,
  c.brand_name,
  c.brand_logo_url,
  ...
  c.ends_at,
  CASE 
    WHEN ucp.current_distance >= c.target_distance THEN true 
    ELSE false 
  END as is_completed,
  ROUND((ucp.current_distance / NULLIF(c.target_distance, 0)) * 100, 1) as progress_percentage,
  ROW_NUMBER() OVER (PARTITION BY ucp.challenge_id ORDER BY ucp.current_distance DESC, ucp.completed_at ASC NULLS LAST) as rank
FROM public.user_challenge_progress ucp
JOIN public.challenges c ON c.id = ucp.challenge_id
JOIN public.profiles p ON p.user_id = ucp.user_id
WHERE c.brand_name IS NOT NULL
  AND c.target_distance IS NOT NULL
  AND c.status = 'active';

-- Enable RLS on the view
ALTER VIEW public.branded_challenge_leaderboard SET (security_barrier = true);

-- Create policy for branded challenge leaderboard view
CREATE POLICY "Everyone can view branded challenge leaderboards" 
ON public.branded_challenge_leaderboard
FOR SELECT 
USING (true);
